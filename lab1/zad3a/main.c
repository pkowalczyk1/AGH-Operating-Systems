...
int main(int argc, char *argv[]) {
    #ifdef DYNAMIC
    void *handle = dlopen("./libwc.so", RTLD_NOW);
    int (*create_table)(int) = dlsym(handle, "create_table");
    int (*wc_files)(char*) = dlsym(handle, "wc_files");
    int (*remove_block)(int) = dlsym(handle, "remove_block");
    int (*save_tmp_to_array)(void) = dlsym(handle, "save_tmp_to_array");
    void (*free_array)(void) = dlsym(handle, "free_array");
    #endif

    FILE *report = fopen("results3a.txt", "a");
    int i = 1;
    while (i < argc) {
        char *command = argv[i++];
